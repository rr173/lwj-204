...
const PIXEL_TO_METER_REPORT = 0.01;
...
function generateReport() {
  ...
  let tableNum = 0;
  const nextTableNum = () => ++tableNum;

  let html = '';

  html += `<div class="report-page">`;
  html += `<h1>桁架结构计算书</h1>`;
  html += `<div class="report-date">生成日期：${dateStr} ${timeStr}</div>`;

  html += `<h2>一、结构概况</h2>`;

  html += `<div class="table-title">表${nextTableNum()}　节点坐标</div>`;
  html += `<table><thead><tr><th>节点号</th><th>X (mm)</th><th>Y (mm)</th><th>支座类型</th></tr></thead><tbody>`;
  nodes.forEach(n => {
    const supportLabel = n.support === 'pinned' ? '固定铰支座' : n.support === 'roller' ? '滚动支座' : '自由';
    html += `<tr><td>${n.id}</td><td>${n.x.toFixed(1)}</td><td>${n.y.toFixed(1)}</td><td>${supportLabel}</td></tr>`;
  });
  html += `</tbody></table>`;

  html += `<div class="table-title">表${nextTableNum()}　杆件连接</div>`;
  ...
  html += `<div class="table-title">表${nextTableNum()}　支座条件</div>`;
  ...
  if (supportNodes.length === 0) {
    ...
    supportNodes.forEach(n => {
      const constraint = n.support === 'pinned' ? 'X, Y' : n.support === 'roller' ? 'Y' : '—';
      const label = n.support === 'pinned' ? '固定铰支座' : '滚动支座';
      html += `<tr><td>${n.id}</td><td>${label}</td><td>${constraint}</td></tr>`;
    });
  }
  html += `</tbody></table>`;

  html += `<div class="table-title">表${nextTableNum()}　材料参数</div>`;
  html += `<table><thead><tr><th>杆件号</th><th>弹性模量 E (GPa)</th><th>截面积 A (cm²)</th></tr></thead><tbody>`;
  members.forEach(m => {
  ...
  const loadedNodes = nodes.filter(n => Math.abs(n.fx) > 1e-6 || Math.abs(n.fy) > 1e-6);
  html += `<div class="table-title">表${nextTableNum()}　节点荷载</div>`;
  html += `<table><thead><tr><th>节点号</th><th>Fx (kN)</th><th>Fy (kN)</th></tr></thead><tbody>`;
  if (loadedNodes.length === 0) {
    html += `<tr><td colspan="3">无外荷载</td></tr>`;
  } else {
  ...
  html += `<div class="table-title">表${nextTableNum()}　节点位移</div>`;
  html += `<table><thead><tr><th>节点号</th><th>dx (mm)</th><th>dy (mm)</th><th>合位移 (mm)</th></tr></thead><tbody>`;
  let maxDisp = 0;
  nodes.forEach(n => {
    const dxMm = n.dx * 1000;
    const dyMm = n.dy * 1000;
    const d = Math.sqrt(dxMm * dxMm + dyMm * dyMm);
    if (d > maxDisp) maxDisp = d;
    html += `<tr><td>${n.id}</td><td>${dxMm.toFixed(4)}</td><td>${dyMm.toFixed(4)}</td><td>${d.toFixed(4)}</td></tr>`;
  });
  html += `</tbody></table>`;
  html += `<p>最大合位移：${maxDisp.toFixed(4)} mm</p>`;

  html += `<div class="table-title">表${nextTableNum()}　杆件内力</div>`;
  html += `<table><thead><tr><th>杆件号</th><th>轴力 (kN)</th><th>性质</th><th>应力 (MPa)</th></tr></thead><tbody>`;
  let maxStress = 0;
  let maxStressMemberId = null;
  members.forEach(m => {
    const force = m.axialForce;
    const stress = m.stress;
    const absStress = Math.abs(stress);
    if (absStress > maxStress) {
      maxStress = absStress;
      maxStressMemberId = m.id;
    }
    const nature = force > 1e-6 ? '拉' : force < -1e-6 ? '压' : '零力';
    html += `<tr><td>${m.id}</td><td>${(force / 1000).toFixed(3)}</td><td>${nature}</td><td>${(stress / 1e6).toFixed(2)}</td></tr>`;
  });
  html += `</tbody></table>`;

  const safetyFactor = maxStress > 0 ? (YIELD_STRESS / maxStress).toFixed(2) : '—';
  html += `<p>最大应力：${(maxStress / 1e6).toFixed(2)} MPa（杆件 #${maxStressMemberId}）</p>`;
  html += `<p>屈服强度：${(YIELD_STRESS / 1e6).toFixed(0)} MPa（Q235钢）</p>`;
  html += `<p>安全系数：${safetyFactor}</p>`;

  html += `<h2>四、结论</h2>`;
  const failedMembers = members.filter(m => Math.abs(m.stress) > YIELD_STRESS);
  ...
  if (failedMembers.length === 0) {
    ...
    html += `<p class="fail">✗ 以下 ${failedMembers.length} 根杆件应力超过屈服强度（${(YIELD_STRESS / 1e6).toFixed(0)} MPa），不满足强度要求：</p>`;
    html += `<ul>`;
    failedMembers.forEach(m => {
      const ratio = (Math.abs(m.stress) / YIELD_STRESS).toFixed(2);
      html += `<li class="fail">杆件 #${m.id}：应力 ${(Math.abs(m.stress) / 1e6).toFixed(2)} MPa，超限比 ${ratio}</li>`;
    });
    html += `</ul>`;
  }
  html += `</div>`;
